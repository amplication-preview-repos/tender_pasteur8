/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { CharacterUpdateManyWithoutTraitSetsInput } from "./CharacterUpdateManyWithoutTraitSetsInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
} from "class-validator";
import { Type } from "class-transformer";
import { TraitUpdateManyWithoutTraitSetsInput } from "./TraitUpdateManyWithoutTraitSetsInput";

@InputType()
class TraitSetUpdateInput {
  @ApiProperty({
    required: false,
    type: () => CharacterUpdateManyWithoutTraitSetsInput,
  })
  @ValidateNested()
  @Type(() => CharacterUpdateManyWithoutTraitSetsInput)
  @IsOptional()
  @Field(() => CharacterUpdateManyWithoutTraitSetsInput, {
    nullable: true,
  })
  characters?: CharacterUpdateManyWithoutTraitSetsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => TraitUpdateManyWithoutTraitSetsInput,
  })
  @ValidateNested()
  @Type(() => TraitUpdateManyWithoutTraitSetsInput)
  @IsOptional()
  @Field(() => TraitUpdateManyWithoutTraitSetsInput, {
    nullable: true,
  })
  traits?: TraitUpdateManyWithoutTraitSetsInput;
}

export { TraitSetUpdateInput as TraitSetUpdateInput };
